...
export interface GatewayDispatcher {
  list(p: Principal): Promise<ToolRecord[]>;
  call(
    p: Principal,
    name: string,
    args: JsonObject,
...
export async function handleMcp(
  ...
  res: Response,
  p: Principal,
  execution: GatewayDispatcher,
) {
  const handler = createMcpHandler(
    async () => {
      const server = new Server(
        { name: 'OmniMCP', version: '0.1.0' },
        { capabilities: { tools: {} }, supportedProtocolVersions: ['2026-07-28'] },
      );
      server.setRequestHandler('tools/list', async () => ({
        tools: (await execution.list(p)).map((t) => ({
          name: t.name,
          description: t.description,
          inputSchema: { ...t.input_schema, type: 'object' as const },
          annotations: {
            readOnlyHint: t.risk === 'READ',
            destructiveHint: t.risk === 'CRITICAL',
            openWorldHint: true,
          },
        })),
      }));
