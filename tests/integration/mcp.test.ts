...
test('official SDK and Inspector discover and call the real Streamable HTTP gateway', async () => {
  ...
  try {
    ...
    expect(listed.tools.map((t) => t.name)).toContain('demo.crm.customer.search');
    const result = await client.callTool({ name: 'demo.crm.customer.search', arguments: {} });
    ...
    async function inspect(args: string[]) {
      return new Promise<string>((resolveResult, reject) => {
        const child = spawn(
          ...
          {
            windowsHide: true,
            env: { ...process.env, MCP_CATALOG_PATH: resolve('.local/inspector-catalog.json') },
        ...
        child.on('close', (code) =>
          code === 0 ? resolveResult(out) : reject(new Error(`Inspector ${code}: ${err} ${out}`)),
        );
      });
    }
    ...
    expect(
      await inspect([
        ...
        'tools/call',
        '--tool-name',
        'demo.crm.customer.search',
        '--tool-args-json',
        '{}',
      ]),
    ).toContain('Ada Nguyen');
    await f.db.system.query("update tools set risk='CRITICAL' where name='demo.crm.note.create'");
    await f.db.system.query(
      "insert into tool_permissions select organization_id,id,'developer',true from tools where name='demo.crm.note.create'",
    );
    expect(
      await inspect([
        '--method',
        ...
        '--tool-args-json',
        JSON.stringify({ customerId: f.customer, body: 'Approval smoke' }),
      ]),
    ).toContain('pending');
  } finally {
    await client.close();
